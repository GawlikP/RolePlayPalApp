                    ...
                    <div></div>
                        <div class="flex min-w-full my-1 col-span-5 xl:col-span-3 lg:col-span-3 md:col-span-5 sm:col-span-5  centered items-center justify-center "> 
                            <div class="lg:my-1 sm:my-8" v-for="index in max_page" :key="index">    
                                    <a  v-on:click=" $emit('updatePage', (parseInt(index) + start_page-1));" v-bind:class="{'text-gray-100 bg-purple-800': parseInt(index)+start_page -1==parseInt(act_page)}" class="w-full px-5 py-2 rounded-xl    hover:bg-purple-600 lg:text-2xl sm:text-xl font-bold shadow-xl" >  
